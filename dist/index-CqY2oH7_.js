import { E as d, a as g, L as v, h as m, p as u, s as x, b, c as G, t as a, i as Z, d as R, f as h, e as k, g as f, j as w } from "./index-0tBP4zDD.js";
const X = 1, T = 2, y = 3, U = 155, z = 4, j = 156;
function Y(O) {
  return O >= 65 && O <= 90 || O >= 97 && O <= 122;
}
const E = /* @__PURE__ */ new f((O) => {
  let i = O.pos;
  for (; ; ) {
    let { next: t } = O;
    if (t < 0)
...
const W = /* @__PURE__ */ _("endraw", j, z), V = { __proto__: null, in: 38, is: 40, and: 46, or: 48, not: 52, if: 78, else: 80, true: 98, false: 98, self: 100, super: 102, loop: 104, recursive: 136, scoped: 160, required: 162, as: 256, import: 260, ignore: 268, missing: 270, with: 272, without: 274, context: 276 }, q = { __proto__: null, if: 112, elif: 118, else: 122, endif: 126, for: 132, endfor: 140, raw: 146, endraw: 152, block: 158, endblock: 166, macro: 172, endmacro: 182, call: 188, endcall: 192, filter: 198, endfilter: 202, set: 208, endset: 212, trans: 218, pluralize: 222, endtrans: 226, with: 232, endwith: 236, autoescape: 242, endautoescape: 246, import: 254, from: 258, include: 266 }, F = /* @__PURE__ */ k.deserialize({
  ...
  nodeProps: [
    ...
    ["group", -18, 9, 10, 13, 14, 21, 22, 25, 27, 38, 41, 42, 43, 44, 45, 49, 50, 51, 52, "Expression", -11, 53, 64, 71, 77, 84, 92, 97, 102, 107, 114, 119, "Statement"]
  ],
  skippedNodes: [0],
  repeatNodeCount: 13,
  ...
  tokenizers: [E, W, 1, 2, 3, 4, 5, /* @__PURE__ */ new w("b~RPstU~XP#q#r[~aO$Q~~", 17, 173)],
  topRules: { Template: [0, 5] },
  specialized: [{ term: 161, get: (O) => V[O] || -1 }, { term: 55, get: (O) => q[O] || -1 }],
  tokenPrec: 3602
});
function S(O, i) {
  return O.split(" ").map((t) => ({ label: t, type: i }));
}
const N = /* @__PURE__ */ S("abs attr batch capitalize center default dictsort escape filesizeformat first float forceescape format groupby indent int items join last length list lower map max min pprint random reject rejectattr replace reverse round safe select selectattr slice sort string striptags sum title tojson trim truncate unique upper urlencode urlize wordcount wordwrap xmlattr", "function"), C = /* @__PURE__ */ S("boolean callable defined divisibleby eq escaped even filter float ge gt in integer iterable le lower lt mapping ne none number odd sameas sequence string test undefined upper range lipsum dict joiner namespace", "function"), D = /* @__PURE__ */ S("loop super self true false varargs kwargs caller name arguments catch_kwargs catch_varargs caller", "keyword"), l = /* @__PURE__ */ C.concat(D), $ = /* @__PURE__ */ S("raw endraw filter endfilter trans pluralize endtrans with endwith autoescape endautoescape if elif else endif for endfor call endcall block endblock set endset macro endmacro import include break continue debug do extends", "keyword");
function A(O) {
  var i;
  let { state: t, pos: r } = O, e = x(t).resolveInner(r, -1).enterUnfinishedNodesBefore(r), P = ((i = e.childBefore(r)) === null || i === void 0 ? void 0 : i.name) || e.name;
  if (e.name == "FilterName")
    return { type: "filter", node: e };
  if (O.explicit && (P == "FilterOp" || P == "filter"))
    return { type: "filter" };
  if (e.name == "TagName")
    return { type: "tag", node: e };
  if (O.explicit && P == "{%")
    return { type: "tag" };
  if (e.name == "PropertyName" && e.parent.name == "MemberExpression")
    return { type: "prop", node: e, target: e.parent };
  if (e.name == "." && e.parent.name == "MemberExpression")
    return { type: "prop", target: e.parent };
  if (e.name == "MemberExpression" && P == ".")
    return { type: "prop", target: e };
  if (e.name == "VariableName")
    return { type: "expr", from: e.from };
  if (e.name == "Comment" || e.name == "StringLiteral" || e.name == "NumberLiteral")
    return null;
...
function I(O = {}) {
  let i = O.tags ? O.tags.concat($) : $, t = O.variables ? O.variables.concat(l) : l, { properties: r } = O;
  return (e) => {
    var P;
    let Q = A(e);
    if (!Q)
      return null;
    let o = (P = Q.from) !== null && P !== void 0 ? P : Q.node ? Q.node.from : e.pos, n;
    return Q.type == "filter" ? n = N : Q.type == "tag" ? n = i : Q.type == "expr" ? n = t : n = r ? L(e.state, Q.target, e, r) : [], n.length ? { options: n, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null;
  };
}
const H = /* @__PURE__ */ d.inputHandler.of((O, i, t, r) => r != "%" || i != t || O.state.doc.sliceString(i - 1, t + 1) != "{}" ? !1 : (O.dispatch(O.state.changeByRange((e) => ({
...
const B = /* @__PURE__ */ b.define({
  name: "jinja",
  parser: /* @__PURE__ */ F.configure({
    props: [
      /* @__PURE__ */ G({
        "TagName raw endraw filter endfilter as trans pluralize endtrans with endwith autoescape endautoescape": a.keyword,
        "required scoped recursive with without context ignore missing": a.modifier,
        self: a.self,
        "loop super": /* @__PURE__ */ a.standard(a.variableName),
        ...
        Definition: /* @__PURE__ */ a.definition(a.variableName),
        PropertyName: a.propertyName,
        FilterName: /* @__PURE__ */ a.special(a.variableName),
      ...
      /* @__PURE__ */ Z.add({
        ...
        "IfStatement ForStatement": /* @__PURE__ */ p(/^\s*(\{%-?\s*)?(endif|endfor|else|elif)\b/),
        Statement: /* @__PURE__ */ p(/^\s*(\{%-?\s*)?end\w/)
      }),
      /* @__PURE__ */ h.add({
        "Statement Comment"(O) {
          let i = O.firstChild, t = O.lastChild;
          return !i || i.name != "Tag" && i.name != "{#" ? null : { from: i.to, to: t.name == "EndTag" || t.name == "#}" ? t.from : O.to };
        }
      })
    ]
  }),
...
function c(O) {
  return B.configure({
    wrap: u((i) => i.type.isTop ? {
      parser: O.parser,
      overlay: (t) => t.name == "Text" || t.name == "RawText"
    } : null)
  }, "jinja");
}
const J = /* @__PURE__ */ c(s.language);
function M(O = {}) {
  let i = O.base || s, t = i.language == s.language ? J : c(i.language);
  return new v(t, [
    i.support,
...
export {
  H as closePercentBrace,
  M as jinja,
  I as jinjaCompletionSource,
  J as jinjaLanguage
};
